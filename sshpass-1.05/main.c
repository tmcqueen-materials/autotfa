...
struct {
    ...
    enum { TFA_NONE, TFA_PRESENT } tfatype;
    union {
	const char *filename;
	int fd;
	const char *password;
...
{
    printf("Usage: " PACKAGE_NAME " [-f|-d|-p|-e] [-htV] command parameters\n"
	    ...
	    "   -h            Show help (this screen)\n"
            "   -t            Modify the behavior of sshpass to be usable with TFA challenges (e.g. Google Authenticator PAM)\n"
            "                 Any password source can be used, the order of inputs should correspond to the order of codes\n"
...
#define VIRGIN_PWTYPE if( args.pwtype!=PWT_STDIN ) { \
    ...
    while( (opt=getopt(argc, argv, "+f:d:p:hetV"))!=-1 && error==-1 ) {
	switch( opt ) {
	case 'f':
...
void write_pass( int fd, int lineno );

int handleoutput( int fd )
{
    // We are looking for the string
    static int prevtfamatch=0,prevmatch=0; // If the "password" prompt is repeated, we have the wrong password.
    static int state0, state1, state2;
    static const char compare0[]="ode:"; // Asking for a verification code
    ...
    if( args.tfatype == TFA_PRESENT ) {
        ...
        if( compare0[state0]=='\0' ) {
            if( !prevtfamatch ) {
                ...
                // Wrong TFA password - terminate with proper error code
...
int match( const char *reference, const char *buffer, ssize_t bufsize, int state )
{
    // This is a highly simplisic implementation. It's good enough for matching "Password: " and "Verification Code: ", though.
    int i;
...
void write_pass_fd( int srcfd, int dstfd, int lineno );

void write_pass( int fd, int lineno )
{
    switch( args.pwtype ) {
    case PWT_STDIN:
        // lineno irrelevant here: should be in order expected by server
	write_pass_fd( STDIN_FILENO, fd, 0 );
	break;
    case PWT_FD:
        // lineno irrelevant here: should be in order expected by server
	write_pass_fd( args.pwsrc.fd, fd, 0 );
	break;
    case PWT_FILE:
	{
	    int srcfd=open( args.pwsrc.filename, O_RDONLY );
	    if( srcfd!=-1 ) {
		write_pass_fd( srcfd, fd, (lineno>0) ? (1) : (0) );
		close( srcfd );
	    }
	}
	break;
    case PWT_PASS:
        {
            const char *pw = NULL;
            pw = args.pwsrc.password;
            if( lineno ) {
                while( pw[0] != '\n' && pw < args.pwsrc.password+strlen(args.pwsrc.password) ) pw++;
                if( pw < args.pwsrc.password+strlen(args.pwsrc.password) ) pw++;
            }
